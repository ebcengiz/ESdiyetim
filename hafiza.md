...
- **Canlı test:** misafir metin → 200 Gemini (3,3 sn); misafir foto → 429 AUTH_SESSION_REQUIRED; geçersiz → 400; verify-subscription/delete-account oturumsuz → 401. Giriş yapmış kullanıcı akışı (Premium/ücretsiz tavan, gerçek StoreKit JWS) canlıda henüz test edilmedi → TestFlight'ta.
- **1.4.2 (15):** Xcode arşivi (commit `fabf91a`) → `xcodebuild -exportArchive` ile ASC'ye yüklendi ("Upload succeeded"). İncelemeye GÖNDERİLMEDİ (1.4.1 Waiting for Review).

### 2026-09-25 — KRİTİK: canlıda herkese açık RLS politikaları bulundu (düzeltme hazır, canlıya UYGULANMADI)
- SQL Editor'de salt-okuma sorgusuyla canlı şema incelendi: `diet_plans`, `weight_records`, `body_info`, `goals` → `"Enable all access for <tablo>"` (FOR ALL, TO public, **USING true**). `supabase-auth-migration.sql`'in DROP POLICY adımları canlıya hiç uygulanmamış. Anon anahtarla (uygulama paketinde, herkese açık) satır sayıları doğrulandı (içerik okunmadan): weight_records 17, diet_plans 6, body_info 14, goals 10 → **tüm kullanıcıların sağlık verisi okunabilir/değiştirilebilir/silinebilir**.
- Düzeltme: `supabase/migrations/20260925140000_fix_open_rls_policies.sql` (açık politikaları kaldırır, `authenticated` + `auth.uid() = user_id` politikaları; `delete_user()` search_path). Uygulamadaki 27 sorgunun hepsi zaten `user_id` filtreli → davranış değişmez. PGlite'ta doğrulandı (anon 0 satır, kullanıcı yalnızca kendi satırı).
- Canlı şema bulguları: `food_logs` ve `user_credits` FK'leri CASCADE; `diet_plans`/`weight_records`/`body_info`/`goals` NO ACTION. `body_info`'da ayrıca `activity_level`, `updated_at` var (anlık görüntü güncellendi). Canlıda `delete_user()` (eski) ve `get_home_daily_summary()` (invoker, auth.uid() ile sınırlı) fonksiyonları var.
- Canlıya uygulama (SQL Editor'den migration çalıştırma) otomatik güvenlik sınıflandırıcısı tarafından engellendi → kullanıcı elle uygulamalı. 1.4.2 (15) Xcode arşivi hazır (`/private/tmp/claude-501/ESdiyet-1.4.2-15.xcarchive`), sunucu tarafı canlıya alınana kadar ASC'ye YÜKLENMEDİ.

### 2026-09-25 — Analiz bulgularının tamamı düzeltildi (commit YOK, sunucu tarafı henüz canlıya UYGULANMADI)
- **AI anahtarları istemciden çıkarıldı:** yeni Edge Function `supabase/functions/ai-proxy` (Gemini → Groq → Cohere → HF; görsel Gemini → Groq). `src/services/ai/providers.js` artık yalnızca `supabase.functions.invoke('ai-proxy')`. Sunucu sistem kuralı (yalnızca beslenme/sağlık, teşhis yok) eklendi. `.env`'de `EXPO_PUBLIC_GEMINI/GROQ_API_KEY` → `GEMINI_API_KEY`/`GROQ_API_KEY` (ön eksiz, yalnızca `supabase secrets set` için). iOS bundle taraması: anahtar ve sağlayıcı URL'i yok ✓.
- **Sunucu tavanı:** `ai_usage` + `ai_usage_consume()` (yalnızca service_role). Kullanıcı: metin 80 / görsel 5 günde; misafir (IP sha256): metin 20 / görsel 0; başarısızlıkta iade.
- **`user_credits` koruması** (`20260925120000_server_side_limits.sql`): `trg_user_credits_guard` (yalnızca `authenticated`/`anon` rolüne) aynı gün düşürme / gelecek tarih / geri tarih reddi; INSERT sıfırdan başlar; atomik `increment_photo_credit()` RPC (istemci yoksa eski UPDATE'e düşer). Yayındaki 1.4.x istemcileriyle geriye uyumlu (PGlite'ta 18 senaryo doğrulandı).
- **Yerel gün:** tüm `toISOString().split('T')[0]` → `toDateString()`; sunucuda `tr_today()` (Europe/Istanbul).
- **IAP:** `restorePurchases` hatayı yutmuyor → geçici StoreKit hatasında Premium düşmüyor; Paywall'da "mağazaya ulaşılamadı" doğru mesajı.
- **Kullanıcıya bağlı yerel veri:** `SubscriptionContext` kredileri `user.id` değişince yeniden yüklüyor; fotoğraf önbelleği `esdiyet_daily_photo_used_v2:<userId>`; çıkış/hesap silmede `clearUserLocalData()` (AI önbelleği dahil).
- **Tips yenileme** önbelleği atlıyor (`getHealthTip(cat, { force })`). **Fotoğraf** 1024 px/JPEG 0.7'ye küçültülüyor (`expo-image-manipulator` — yeni native modül, prebuild gerekir).
- **Şema:** `20260925110000_schema_snapshot_food_logs_body_info.sql` (IF NOT EXISTS); `delete_own_account` + `delete-account` Edge Function `food_logs`/`user_credits`/`ai_usage`'ı da siliyor.
- **Düşük öncelik:** `PressableOpacity` (UI kiti) ile 22 `TouchableOpacity` değiştirildi; `FoodSearchModal` kaydetme hatası `handleError`'a; `includes()` ile hata tanıma → `isUniqueViolation`/`isMissingConflictTarget`/`isMissingRpc`; 8 Expo paketi patch güncellendi; `app.config.js` fonksiyon biçimi (expo-doctor 21/21); tsconfig `supabase/functions` hariç; `package.json` 1.4.1.
- **Kalite altyapısı:** ESLint (`eslint.config.js`, 0 hata — React Compiler kuralları uyarı), Jest (`__tests__/`, 16 test), GitHub Actions CI (`.github/workflows/ci.yml`).
- **Doküman:** CLAUDE.md/AGENTS.md (AGENTS.md CLAUDE.md'den yeniden üretildi — eskimişti), `NSPrivacyTracking: false` gerekçesi, `GROQ_KURULUM.md` proxy kurulumuna göre yeniden yazıldı (eskisi anahtarı koda yapıştırmayı öğretiyordu), `PRIVACY.md` + `PrivacyPolicyScreen` aracı fonksiyon/istek sayacı cümlesi.

### 2026-09-25 — Detaylı analiz tekrarı (kod değişikliği yok)
- Sabah analizindeki bulguların hepsi **hâlâ açık** (AI anahtarları bundle'da, istemci tarafı limitler + `user_credits` UPDATE policy, UTC gün dönümü, Tips yenileme önbelleği, fotoğraf boyutlandırma yok, SQL şema kayması).
- **Yeni:** (1) `restorePurchases` hatayı yutup `[]` dönüyor → `loadSubscriptionStatus` geçici StoreKit/ağ hatasında ödeme yapan kullanıcıyı ücretsize düşürüp cache'e `false` yazıyor. (2) `SubscriptionContext` kullanıcıdan bağımsız: krediler yalnızca mount'ta yükleniyor, giriş/hesap değişiminde yenilenmiyor; `esdiyet_daily_photo_used_v1` / `esdiyet_sub_status_v1` / AI önbelleği kullanıcıya göre ayrılmıyor ve çıkışta temizlenmiyor. (3) CLAUDE.md §9.11(d) `NSPrivacyTracking: true` diyor, `app.json` bilinçli olarak `false` (ITMS-91064) — doküman kayması. (4) `expo-doctor`: 8 paket geride, 2 kontrol başarısız.
...
- **Sürüm:** `app.json` 1.4.1 / build 14, `expo prebuild --clean` (Info.plist 1.4.1/14, gerçek GAD App ID). Commit `c663457`. Xcode archive (1.4.1/14, gerçek GAD App ID + 2 gerçek ad unit bundle'da doğrulandı) → `xcodebuild -exportArchive` ile ASC'ye yüklendi (hermesvm dSYM uyarısı zararsız) → build 14 "Ready to Submit", ESdiyet Test grubunda.
- **ASC:** 1.4.1 sürümü oluşturuldu; What's New (TR, 3 madde: reklam altyapısı, rıza yalnızca kişiselleştirme, gizlilik metni) yazıldı; Review Notes'taki "Before the first ad, an in-app choice is shown" cümlesi 1.4.1 davranışıyla değiştirildi (2342 karakter); build 14 bağlandı → **Submit for Review → "1.4.1 Waiting for Review"** (25 Eyl ~02:00 TR). Otomatik yayın açık.
- **Sonraki:** (1) App Review (≤48 saat). (2) TestFlight build 14'te giriş yapmış ücretsiz hesapla doğrula: açılışta istek gitmeli, limit dolunca "Reklam izle" görünmeli (gerçek ID — tıklama yok). (3) ~1 Ekim AdMob Verify app → "Güncellemeleri kontrol edin". (4) Apple Developer Program 5 Ekim'de bitiyor — yenile.
...
**Çözüm — Xcode archive + doğrudan yükleme** (bkz. [[ios-release-toolchain]] memory'si, 1.3.2/1.3.3 turlarında da kullanılmıştı): `npx expo prebuild --clean` zaten yapılmıştı (build 13 doğrulaması sırasında). `xcodebuild archive` ilk denemede **"Signing requires a development team"** hatası verdi (Expo prebuild pbxproj'a `DEVELOPMENT_TEAM` yazmıyor) → `DEVELOPMENT_TEAM=AVL8UT8FA8 CODE_SIGN_STYLE=Automatic` build setting'leri komut satırından geçilerek düzeltildi. Arşiv ~13 dakikada başarıyla tamamlandı. `xcodebuild -exportArchive` (`method: app-store-connect`, `destination: upload`, `teamID: AVL8UT8FA8`, `signingStyle: automatic`) ile **doğrudan App Store Connect'e yüklendi** — "Upload succeeded" (hermesvm dSYM uyarısı zararsız, bilinen). EAS'daki iki takılı submission (`ec04d2a4`, `abd0dcf1`) sonuçsuz kaldı, iptal edilmedi (kendiliğinden zaman aşımına uğrayabilir).

**Sonuç:** Apple ~10 dakikada işledi → Build 13 TestFlight'ta "Ready to Submit" oldu. ASC'de 1.4.0 sürümünden eski geçersiz build 12 kaldırıldı (Build bölümü → kırmızı "remove" ikonu), build 13 eklendi (Add Build → radio seçimi, **dikkat: modal'da screenshot `scale` parametresi 1'e ayarlanmalı, `scale<1` ile alınan görsel üzerinden koordinat hesaplarken tam çözünürlük çerçevesine göre ölçeklendirilmemiş tıklama satır kaydırdı — bir kez yanlışlıkla build 12 yeniden seçildi, ekran görüntüsü `scale:1` ile tekrarlanarak düzeltildi**) → Save → **Resubmit to App Review** → **"1.4.0 (13) — Waiting for Review"** (22 Eylül 22:29).

**Sonraki adımlar (değişmedi, bkz. önceki girdi):** (1) App Review sonucu (48 saate kadar). (2) 1.4.0 yayınlanınca AdMob → Verify app → "Güncellemeleri kontrol edin" (Marketing URL kataloğa yansır, "Sınırlı reklam sunumu" kalkar). (3) Gerçek cihazda TestFlight build 13 ile reklam/ATT akışını gözle doğrula. (4) ASC Privacy Policy URL hâlâ `ebcturkiye.com/iletisim` — gerçek gizlilik politikası sayfasına çevrilmeli.

### 2026-09-22 (gece/sabah) — Build 12 reddedildi (ITMS-91064) → build 13 ile düzeltildi; AdMob "Sınırlı reklam sunumu" nedeni doğrulandı

**ITMS-91064 (Apple reddi, build 12):** "NSPrivacyTracking must be true if NSPrivacyTrackingDomains isn't empty." App-level `PrivacyInfo.xcprivacy`'de `NSPrivacyTracking: true` + `NSPrivacyTrackingDomains: []` (boş) kombinasyonu Apple'ın statik ITMS doğrulamasını geçemiyor. Araştırma (GitHub issue'ları, Apple forum, AdMob SDK grup tartışması) net: ya `NSPrivacyTracking:true` iken gerçek alan adları listelenmeli, ya da domain yoksa `false` yapılmalı. AdMob/UMP SDK'larının kendi `PrivacyInfo.xcprivacy` dosyaları da `NSPrivacyTrackingDomains` beyan **etmiyor** (Pods içinde doğrulandı — hem Google-Mobile-Ads-SDK hem GoogleUserMessagingPlatform); uygulama kodu da doğrudan bir izleme alan adına bağlanmıyor (SDK içeride hallediyor). Listelenecek gerçek bir alan adı olmadığı için **`app.json` → `ios.privacyManifests.NSPrivacyTracking: false`** yapıldı (commit `ff51a3b`). Bu değişiklik ATT akışını (NSUserTrackingUsageDescription + runtime `requestTrackingPermissionsAsync`) ve ASC App Privacy'deki "Used for tracking purposes" (Device ID, Advertising Data) beyanlarını **etkilemez** — ikisi ayrı sistem; bu yalnızca ITMS'in bundled-resource statik tutarlılık kontrolü.

**Build 13:** `npx expo prebuild --clean` ile yerel doğrulama (`PrivacyInfo.xcprivacy` → `NSPrivacyTracking=false`, `NSPrivacyTrackingDomains=[]` — artık çelişkisiz), `pod install` temiz → EAS production build **13** (`df284531-…`, commit `ff51a3b`) alındı, `eas submit --wait` ile ASC'ye yükleniyor. ASC'de 1.4.0 submission'ı "Unresolved Issues / Rejected (build 12)" durumunda — build 13 işlenince Distribution → 1.4.0 → Add Build (13) → Save → Resubmit to App Review yapılacak.

**AdMob "Sınırlı reklam sunumu" / TestFlight'ta reklam çıkmıyor — kök neden doğrulandı:** AdMob konsolu hâlâ "ESdiyet (iOS) uygulamasını doğrulayamadık" diyor çünkü `itunes.apple.com/lookup?id=6753659091` **hâlâ `sellerUrl: null`, `version: 1.3.3`** döndürüyor — Marketing URL (`ebcturkiye.com/iletisim`) yalnızca ASC taslağında var, Apple'ın **canlı** kataloğuna 1.4.0 onaylanıp yayınlanana kadar yansımayacak. Bu yüzden AdMob uygulamayı doğrulayamıyor → **"Sınırlı reklam sunumu"** statüsü sürüyor → genel AdMob pratiğinde doğrulanmamış/yeni hesaplarda gerçek (TestId olmayan) reklam birimlerinde sıklıkla no-fill görülür. Dashboard'da bugüne kadar **0 istek / 0 gösterim / 0 kazanç** — sunucuya istek gitmiş ama karşılığında reklam dönmemiş (kod tarafında bu durum kasıtlı olarak sessiz: `adsService.js` no-fill'i `console.warn` ile loglar, `LimitReachedSheet`'teki "Reklam izle" butonu yalnızca reklam gerçekten yüklüyse render edilir). **TestFlight build'leri (11/12/13) `eas.json`'daki `production` profiliyle derleniyor** (`preview` değil) → `EXPO_PUBLIC_IS_TESTFLIGHT` set değil → `isTestEnv=false` → `adsService.resolveUnitIds()` Google'ın garantili-dolum `TestIds`'i değil **gerçek üretim ad unit ID'lerini** kullanıyor (bilinçli tasarım: TestFlight gerçek kullanıcı deneyimini yansıtsın diye) — bu da doğrulanmamış hesapta no-fill riskini artırıyor. **Kod tarafında bir hata yok**; sıralama meselesi: 1.4.0 yayınlanır → Marketing URL kataloğa yansır → AdMob "Verify app" geçer → sınırlı sunum kalkar → gerçek reklam trafiği başlar (yeni hesapta ilk günlerde yine de düşük dolum normal).

**AdMob konsol doğrulaması (bu turda tekrar kontrol edildi, değişiklik yok — sağlam):** 2 aktif reklam birimi ID'leri `.env`/EAS ile birebir eşleşiyor (`…/2029144336` geçiş, `…/2690208059` ödüllü); Engelleme kontrolleri hâlâ 13/15 hassas kategori engelli + içerik derecesi **PG**; Politika merkezinde ihlal yok; Ödeme hesabı (AdSense Türkiye) bağlı, uyarı yok.

**ASC doğrulaması:** App Privacy (12 veri tipi, Device ID/Advertising Data "tracking") hâlâ yayında ve doğru; App Information → Bundle ID `com.esdiyet.app`, kategori **Health & Fitness / Food & Drink**, yaş derecelendirmesi 12+ (bölgesel istisnalarla 13+/vb.) — AdMob'daki PG ile tutarlı. **Kalan not (değişmedi):** Privacy Policy URL alanı hâlâ `ebcturkiye.com/iletisim` (iletişim sayfası) — gerçek bir gizlilik politikası sayfasına çevrilmeli, PRIVACY.md siteye eklenmeli.

### 2026-09-22 — 1.4.0: Ücretsiz plana AdMob reklamları (ödüllü + günde 1 geçiş), KVKK rıza akışı, ATT, rehber dokümanı

**Neden:** Paywall "reklamsız, sınırsız erişim" vaat ediyordu ama uygulamada reklam yoktu. Sıfır bütçeyle yalnızca ücretsiz kullanıcıya, UX'i boğmayan reklam + Apple/KVKK uyumu istendi. Kullanıcı kararları: ödüllü + günde ≤1 geçiş, sadece Türkiye (UMP yok), sürüm **1.4.0 (build 10)**; incelemedeki 1.3.3 (9) bekletiliyor.

**Reklam modeli (kod: `src/contexts/AdsContext.js`):** `adsEnabled = native modül var && !isSubscribed`. (1) **Geçiş reklamı** AI isteği gönderildikten sonra *yükleme süresinde* (`showInterstitialIfEligible`), günde en fazla 1 — fotoğraf + besin akışları için ortak sayaç `ad_interstitial`. (2) **Ödüllü reklam** günlük hak dolunca `LimitReachedSheet` (`components/ads/`): "Reklam izle, +1 analiz" butonu **yalnızca reklam yüklüyse** görünür (no-fill'de ölü buton yok → App Review 2.1), özellik başına günde ≤2 ödül (`ad_reward:photo|food`). Bonus cihaz-yerel: `dailyUsageService.getDailyBonus/addDailyBonus` (`<key>:bonus`), `hasReachedDailyLimit` bonusu otomatik ekliyor; fotoğraf için `SubscriptionContext.dailyLimit = 1 + bonusPhotoCredits`. DB migration yok. Banner yok.

**Rıza (KVKK + ATT):** `AdConsentModal` (global, AdsProvider içinde) ilk reklamdan önce: *Kişiselleştirilmiş* → `expo-tracking-transparency` ATT prompt'u; *Sadece genel* → `requestNonPersonalizedAdsOnly: true`. Karar `esdiyet_ad_consent_v1 {decided, personalized, date, version}` (`adConsentService.js`). Rıza yoksa akış bloklanmaz (o seferlik reklamsız). Profil → "Kişiselleştirilmiş reklamlar" switch'i (yalnızca ücretsizde). Sağlık verisi reklam ağına gitmez — `adsService.js` `keywords/contentUrl` kullanmaz (CLAUDE.md §9.11).

**Teknik:** `react-native-google-mobile-ads@16.5.0` (v17.0.0 18 Eyl'de çıktı, API yenilendi — bilinçli alınmadı) + `expo-tracking-transparency ~57.0.2`. Plugin'ler **`app.config.js`'de** (App ID `.env` `EXPO_PUBLIC_ADMOB_IOS_APP_ID`, yoksa Google test App ID; ad unit'ler `EXPO_PUBLIC_ADMOB_INTERSTITIAL_ID/REWARDED_ID`; ATT metni; Google'ın 50 SKAdNetwork ID'si; `delayAppMeasurementInit`). `app.json`: 1.4.0 / build 10, `NSPrivacyTracking: true` + DeviceID/AdvertisingData/ProductInteraction/CoarseLocation/CrashData/PerformanceData. `environment.js`: `EXPO_PUBLIC_BYPASS_PAYWALL=false` ile test ortamında limit/ödül akışı denenebilir. `errorMessages`: `AD_UNAVAILABLE`. `npm run env:check` AdMob satırlarını da listeliyor. Provider sırası: `… SubscriptionProvider → AdsProvider → ToastProvider`. `SubscriptionContext.navigateTo(route)` eklendi (global modal → Gizlilik politikası).

**Metinler:** `PrivacyPolicyScreen`, `PRIVACY.md` ("Reklamlar" + "KVKK Hakları" bölümleri, "reklam yapılmaz" cümleleri düzeltildi), `HealthSourcesInfoScreen`, Paywall alt başlığı, Profil üyelik kartı. **Rehber:** `REKLAM_ENTEGRASYON_REHBERI.md` — AdMob konsol adımları (blocking controls!), ATT/PrivacyInfo/ASC App Privacy tablosu, Review Notes metni, KVKK 11 maddelik liste (avukat teyidi gerekenler işaretli), test matrisi.

**Doğrulama:** Metro `expo export --platform ios` başarılı; `npx expo prebuild --clean` + `pod install` temiz (Google-Mobile-Ads-SDK 13.5.0, GoogleUserMessagingPlatform 3.1.0, ExpoTrackingTransparency 57.0.2); `ios/ESdiyet/Info.plist`'te `GADApplicationIdentifier` (test ID), `GADDelayAppMeasurementInit`, `NSUserTrackingUsageDescription`, 50 `SKAdNetworkItems`, `CFBundleShortVersionString 1.4.0 / CFBundleVersion 10`; `PrivacyInfo.xcprivacy` `NSPrivacyTracking=true` + 12 veri tipi. `xcodebuild` Debug simülatör derlemesi **BUILD SUCCEEDED**; uygulama iPhone 17 simülatöründe açıldı (Login ekranı, çökme yok). Rıza sheet'i / test reklamı / ödül akışı **elle test edilmedi** (giriş + `EXPO_PUBLIC_BYPASS_PAYWALL=false` gerekiyor — rehber §6).

**AdMob konsolu (aynı gece, Chrome ile):** Uygulama AdMob'da kayıtlı: App ID `ca-app-pub-2213399330903197~2847401984`. Oluşturulan ad unit'ler: geçiş **`…/2029144336`** ("ESdiyet iOS - AI analiz gecis"), ödüllü **`…/2690208059`** ("ESdiyet iOS - Ek analiz odullu", 1×analysis). Üçü `.env`'e yazıldı (`npm run env:check` ✓); **EAS production ortamına henüz eklenmedi** (`eas env:create`, rehber §3.2). Engelleme kontrolleri: 13/15 hassas kategori engellendi (astroloji + yardımcı programlar açık), hesap içerik derecesi **PG** (AdMob tahmini: gösterim −29…−57 %, gelir −31…−64 % — gerekirse T'ye gevşetilebilir). **Uygulama doğrulama:** `https://ebcturkiye.com/app-ads.txt` canlı (repo `ebcengiz/ebc`, commit `b41b01b`). Yayındaki 1.3.3'te Marketing URL alanı Apple tarafından kilitli (Support URL zaten `https://ebcturkiye.com/`). Bu yüzden ASC'de **1.4.0 sürümü oluşturuldu ("Prepare for Submission")** ve Marketing URL = `https://ebcturkiye.com/iletisim` kaydedildi; build bağlanmadı, What's New boş. AdMob doğrulaması ancak **1.4.0 App Store'da yayınlanınca** (Apple kataloğunda `sellerUrl` dolunca) geçer → o zaman AdMob → Apps → ESdiyet → Verify app → "Güncellemeleri kontrol edin". Şimdilik "sınırlı reklam sunumu" statüsü normal.

**Yayın turu (aynı gece, devam):** Kod commit'lendi (`67c5c55` feat, `4d26f4d` fix). EAS production env'e `EXPO_PUBLIC_ADMOB_*` (3) eklendi (`eas env:create`). EAS remote build sayacı 10'a çekildi → build **11** (`c20326a3`, commit 67c5c55; `--auto-submit` ASC'ye ulaşmadı, paralel oturum `eas submit --id` ile yükledi → TestFlight'ta "Ready to Submit") ve build **12** (`5ba6cade`, commit 4d26f4d — consent host düzeltmesini içeren asıl aday). **ASC:** App Privacy yayınlandı (12 veri tipi; Device ID + Advertising Data "tracking", Product Interaction/Coarse Location/Crash/Performance eklendi). 1.4.0 sürümünde What's New (TR) ve Review Notes (1.4.0 reklam/ATT açıklaması + tetikleme adımları) kaydedildi. **Simülatör E2E (iPhone 17, Metro `EXPO_PUBLIC_BYPASS_PAYWALL=false`, demo hesap):** rıza sheet'i → gerçek ATT prompt'u (TR metin) → Google test interstitial AI yüklenirken → `ad_interstitial=1` → besin sayacı 3'e set edilince `LimitReachedSheet` + "Reklam izle" → test rewarded → "Ödül verildi" → `food_ai_search:bonus=1`, `ad_reward:food=1`. **Bulunan ve düzeltilen hata:** FoodSearchModal bir RN Modal olduğu için global AdConsentModal üstünde açılmıyordu → `AdsContext.registerConsentHost`/`consentPrompt` ile modal içinde render (fix `4d26f4d`). **Bilinen kısıt:** ToastProvider modal dışında → FoodSearchModal açıkken toast'lar (hata/ödül) görünmüyor (reklamdan bağımsız, eski davranış). Not: simülatörde `esdiyet_sub_status_v1=true` cache'i (eski paywall testi) reklamı kapatıyordu — AsyncStorage'dan `false` yapıldı; simülatör klavyesi US'e çevrildi (idb metin girişi için). Gerçek cihaz testi yapılmadı (cihaz yok) — TestFlight build 11/12 ile kullanıcı doğrulamalı.
**Sonuç (02:50):** Build 12 Apple'da işlendi → ASC'de 1.4.0'a bağlandı → **Submit for Review → "1.4.0 Waiting for Review"** (build 12, commit `4d26f4d`). Build 11 TestFlight'ta kalıyor (consent host düzeltmesi yok, sadece iç test). **Kalan / takip:** (1) App Review sonucu (48 saate kadar; ret gelirse Review Notes'taki tetikleme adımlarına ve ATT açıklamasına bak). (2) 1.4.0 yayınlanınca AdMob → Apps → ESdiyet → Verify app → "Güncellemeleri kontrol edin" (Marketing URL o zaman kataloğa yansır). (3) Gerçek cihazda TestFlight build 12 ile rıza/ATT/reklam akışını gözle doğrula (production ID'ler: reklamlara tıklama!). (4) ASC Privacy Policy URL hâlâ `https://ebcturkiye.com/iletisim` — gerçek gizlilik politikası sayfasına çevrilmeli (PRIVACY.md'yi siteye koymak gerekir). (5) FoodSearchModal açıkken toast'lar görünmüyor (ToastProvider modal dışında) — ayrı UX işi.
...
**Yapılanlar:**
1. **`theme.js` genişletildi:** `COLORS.white/black/transparent`, durum zeminleri (`successBg/warningBg/errorBg/infoBg` + `*Text`), nötr ölçek (`neutral900…100`), kategori vurguları `COLORS.accents.{indigo,violet,pink,coral,amber,emerald,teal,sky}`; `withAlpha(hex, a)` / `whiteAlpha(a)` / `blackAlpha(a)` yardımcıları (`'#fff'`, `rgba(255,255,255,0.2)`, `COLORS.primary + '22'` kalıplarının yerine); `SIZES.minTouch=44`, `buttonHeight`, `inputHeight`, `smallScreenWidth`; `HIT_SLOP`, `MAX_FONT_SCALE=1.3`; `TYPOGRAPHY.label/button/small`; **`LAYOUT.tabBar` + `tabBarMetrics(insetsBottom)`** — tab bar ölçüleri tek kaynak, `scrollTabScreenBottomPad()` artık sihirli sayı (108/96) yerine buradan türüyor (imza aynı, 6 mevcut çağrı bozulmadı).
2. **`hooks/useResponsive.js`:** `useWindowDimensions` + safe area → `width, height, fontScale, isSmall, isLarge, contentWidth, columnWidth(n), scale(), tabBottomPad, bottomPad, topPad`. Modül seviyesi `Dimensions.get` kalıbının yerine (7 dosyada hâlâ var — Adım 3'te taşınacak).
3. **Ortak UI kiti `src/components/ui/` (barrel `index.js`):** `AppButton` (Pressable + haptic + gradient primary + 8 varyant + loading + erişilebilirlik), `AppInput` (etiket, ikon, inline hata, odak halkası, şifre göz butonu, birim), `AppCard` (başlık/ikon/sağ eylem, basılabilir), `IconBadge`, `SectionHeader`, `EmptyState`, `ErrorState` (AppError'dan başlık/mesaj/ikon + Tekrar dene), `LoadingState` (spinner/list/card/inline skeleton), `ScreenContainer` (safe area + scroll + tab alt boşluğu + klavye + pull-to-refresh + sabit header/footer), `BottomSheet` (animasyonlu, klavye uyumlu, handle + başlık + footer), `OfflineBanner`.
4. **Toast v2:** `ToastContext` kuyruklu (art arda mesajlar üst üste binmez, aynı metin tekrarlanmaz), eylem butonu (`showToast(msg, type, { action })`), dokununca kapanır, `accessibilityRole="alert"`, tüm renkler temadan. `useAppError.handleError(e, { onRetry })` → retryable hatada otomatik "Tekrar dene".
5. **Mevcut bileşenler kite taşındı:** `ConfirmModal` (AppButton + IconBadge, `loading` prop'u eklendi), `DatePickerSheet` (BottomSheet üstünde, `title/min/maxDate` prop'ları), `GuestGateBanner`, `ErrorBoundary`. Hardcoded renkler bu dosyalardan temizlendi.
6. **Tab bar (`MainNavigator`):** yükseklik/konum `tabBarMetrics(insets.bottom)` ile runtime'da; iOS'ta `expo-blur` cam zemin (`tabBarBackground`), Android yarı saydam beyaz; `Dimensions.get` kaldırıldı (`useResponsive`); yükleme ekranı `LoadingState`.
7. `App.js`'e `<OfflineBanner />` eklendi (ToastProvider içinde, navigator üstünde).

**Kritik bulgu — netinfo & Metro:** `@react-native-community/netinfo` native modül yoksa **import anında** throw ediyor; yeniden build alınmamış dev build'de uygulama açılışta LogBox "Uncaught Error" verdi (v1.3'teki açılış çökmesi sınıfı). İlk deneme (runtime `require` + try/catch) İŞE YARAMADI: Metro, ilk bundle yüklemesi dışında yapılan `require` hatalarını throw etmek yerine `ErrorUtils.reportFatalError` ile raporluyor (`metro-runtime/src/polyfills/require.js` → `guardedLoadModule/inGuard`). Çözüm: require'ı `connectivity.js` **modül kapsamında** try/catch içine almak (ilk yükleme korumalı bölgede → normal throw → yakalanır). Simülatörde doğrulandı: eski build netinfo'suz açılıyor, sadece `console.warn`. CLAUDE.md §9-9'a kural eklendi.

**Kritik bulgu 2 — `tabBarBackground` + Fabric:** react-navigation `tabBarBackground` öğesini `absoluteFill` bir sarmalayıcıya koyuyor; öğenin kendisi de `absoluteFill` olunca Fabric'te **sıfır boyut** alıyor (görünmez — kırmızı/mavi test View'larıyla simülatörde doğrulandı; `flex: 1` verilince görünüyor). Çözüm: `TabBarBackground` sarmalayıcısı `flex: 1`, içindeki `BlurView` `flex: 1`. Ayrıca blur üstüne beyaz overlay (absolute) da aynı nedenle görünmediğinden `tint="systemThickMaterialLight"` + container'da `whiteAlpha(0.45)` zemin kullanıldı.

**Doğrulama:** 24 dosya babel syntax-check; `npx expo export --platform ios` temiz; `npx expo run:ios` ile netinfo native modülünü içeren yeni dev build alındı (Build Succeeded); simülatörde (iPhone 17 Pro) Login, Home (GuestGateBanner yeni `AppButton` ile) ve **yeni tab bar** (safe area'ya göre yüzen, cam beyaz zemin, 6 sekme) ekran görüntüsüyle doğrulandı.

**Not (simülatör):** Bu makinede Simulator.app GUI'si bulunamadı (Xcode 27; `xcrun simctl` headless çalışıyor, ekran görüntüsü alınabiliyor ama tap gönderilemiyor). Tab'lara ulaşmak için misafir modu AsyncStorage manifest'ine (`Library/Application Support/com.esdiyet.app/RCTAsyncLocalStorage_V1/manifest.json`) elle yazıldı (`ESDIYET_GUEST_MODE_V1=1`).
...
- **[ÇÖZÜLDÜ 2026-09-25, tsconfig exclude]** ~~**`supabase/functions/delete-account/index.ts`** proje kök `tsc --noEmit` taramasına dahil oluyor ve Deno globalleri (`Deno`, esm.sh import'ları) yüzünden tip hatası veriyor. Fonksiyonel bir sorun değil (Deno edge function, ayrı runtime), ama `tsconfig.json`'da `exclude` ile ayrılması temiz olur.~~
...
- [x] ~~ACİL — RLS açığı~~ (2026-09-25 canlıda kapatıldı, anon 0 satır doğrulandı)
- [x] ~~**Sunucu tarafını canlıya uygula**~~ (2026-09-25 tamamlandı) — (1) SQL Editor'de `20260925110000_…`, `20260925120000_…`, `20260925130000_…` migration'ları; (2) Edge Functions secrets: `GEMINI_API_KEY`, `GROQ_API_KEY`; (3) `ai-proxy` + `verify-subscription` deploy (Verify JWT kapalı) + `delete-account` yeniden deploy. Bkz. `GROQ_KURULUM.md`. Yayındaki 1.4.1 bundan etkilenmez (geriye uyumlu).
