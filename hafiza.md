...
- [ ] **ACİL — `20260925140000_fix_open_rls_policies.sql`'i canlıya uygula** (herkese açık sağlık verisi).
- [ ] **Sunucu tarafını canlıya uygula — yeni uygulama build'inden ÖNCE:** (1) SQL Editor'de `20260925110000_…`, `20260925120000_…`, `20260925130000_…` migration'ları; (2) Edge Functions secrets: `GEMINI_API_KEY`, `GROQ_API_KEY`; (3) `ai-proxy` + `verify-subscription` deploy (Verify JWT kapalı) + `delete-account` yeniden deploy. Bkz. `GROQ_KURULUM.md`. Yayındaki 1.4.1 bundan etkilenmez (geriye uyumlu).
- [ ] Yeni sürüm (1.4.2): `expo prebuild --clean` (expo-image-manipulator native), TestFlight'ta AI metin/foto, Tips yenileme, çıkış-giriş sayaç, restore akışı elle doğrulanmalı.
